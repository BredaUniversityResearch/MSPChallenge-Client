    ...
    public static class FortunesAlgorithm
    {
        public static LinkedList<VEdge> Run(List<FortuneSite> sites, double minX, double minY, double maxX, double maxY)
        {
            ...
            while (edgeNode != null)
            {
                ...
                var valid = ClipEdge(edge, minX, minY, maxX, maxY) && HasValidLength(edge);
                if (!valid)
        ...
        private static bool HasValidLength(VEdge edge)
        {
            double startMagSqr = edge.Start.X * edge.Start.X + edge.Start.Y * edge.Start.Y;
            double endMagSqr = edge.End.X * edge.End.X + edge.End.Y * edge.End.Y;
            return startMagSqr != endMagSqr;
        }
